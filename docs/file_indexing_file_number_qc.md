...
2. `_run_qc_validation` skips blank numbers, otherwise evaluates three rule sets:
  padding, year, and spacing.
...
| `spacing` | Any internal whitespace inside the core file number (parenthetical suffixes are ignored). | `KNS - 2024 - 5` → `KNS-2024-5` | Medium |
...
- **Suggested fix:** Drop the leading zeros while keeping any trailing suffix.
- **Example:**
  - Raw: `KN-2023-00045`
...
- **Pattern:** Any whitespace within the core file number. Trailing parenthetical
  suffixes (for example, `(AMENDED)`) are tolerated.
- **Suggested fix:** Collapse whitespace to single hyphens (preserving existing
  hyphenated segments) and keep any trailing suffix untouched.
- **Example:**
  - Raw: `KNS  -  2024  12`
...
- For padding and year issues, double-check that suggested fixes keep any
  trailing parenthetical content intact.
