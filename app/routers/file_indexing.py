...
def _process_import_data(session_data: Dict[str, Any], progress_key: str = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "message": f"Successfully imported {imported_count} file indexing records" + (f" and processed {cofo_count} CofO records" if cofo_count else '')
    }
...
async def clear_file_indexing_data(request: ClearDataRequest):
    ...
    try:
        ...
        counts = {
            ...
            "entities_staging": db.query(EntityStaging).filter(EntityStaging.test_control == mode).delete(synchronize_session=False),
            "customers_staging": db.query(CustomerStaging).filter(CustomerStaging.test_control == mode).delete(synchronize_session=False),
